...
```{r}
raster_list <- list.files(path = "01_datos/soilgrids", full.names = TRUE)
```
...
```{r}
ca <- read_stars("01_datos/prueba/wc2.0_bio_30s_01.tif")
```


```{r}
cr_02 <- st_read("01_datos/mask/CR_cg.gpkg")
```
```{r}
rasters_mask <- ca[cr_02, crop = TRUE]
```
