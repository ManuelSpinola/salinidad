...
```{r}
ph_0_30_sp <- st_as_sf(ph_0_30, coords = c("x", "y"), crs = 4326)
```
...
```{r}
s_spdf <- as(s, "SpatialPixelsDataFrame")
```


```{r}
m <- train.spLearner(meuse["lead"], covariates = s_spdf[,c("dist","ffreq")], lambda = 1)
```
